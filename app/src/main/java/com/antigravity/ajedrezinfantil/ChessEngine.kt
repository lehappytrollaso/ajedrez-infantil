...
enum class SparkyDifficulty {
    TODDLER,   // Aprendiz / Baby steps: gentle, never captures Queen, frequent mistakes
    FRIEND,    // Amigo: balanced instructional play
    CHAMPION   // Campeona: alert, tactical, avoids blunders
}
...
class ChessGame {
    ...
    // --- Minigame 1: Guerra de Peones (Pawn Wars) ---
    fun setupPawnWars() {
        clearBoard()
        for (c in 0..7) {
            board[1][c] = Piece(PieceType.PAWN, PieceColor.BLACK)
    ...
    // --- Minigame 2: El Festín del Caballo (The Hungry Knight) ---
    ...
    // Hint Generator for White (Player)
    fun getBestHint(): Move? {
        ...
        for (m in legalMoves) {
            ...
            // Reward central moves
            val distToCenter = abs(m.to.row - 3.5) + abs(m.to.col - 3.5)
            score += ((7.0 - distToCenter) * 5).toInt()

            // Advancing pieces out of starting rank
